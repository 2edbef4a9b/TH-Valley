...
void TiledMap::initCitizenPosition() {
    ...
    std::uniform_int_distribution<> MaxWidth(
        ...
        tiled_map_->getMapSize().width * tiled_map_->getTileSize().height * 3 /
            4);
    for (int i = 0; i < MapCitizens.size(); i++) {
        ...
        SpritetoCitizen[MapCitizens[i]->initSprite] = MapCitizens[i];
        CitizensSprite.push_back(MapCitizens[i]->initSprite);
        MapCitizens[i]->initSprite->setPosition(
            cocos2d::Vec2(RandomWidth, RandomHeight));
        CCLOG("Citizen at: %d %d\n", RandomWidth, RandomHeight);
        tiled_map_->addChild(MapCitizens[i]->initSprite);
    }
    
    this->schedule(CC_SCHEDULE_SELECTOR(TiledMap::updateCitizenSprites), 5.0f);
}

void TiledMap::updateCitizenSprites(float dt) {
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_int_distribution<> MaxHeight(
        tiled_map_->getMapSize().height * tiled_map_->getTileSize().height / 4,
    ...
    for (int i = 0; i < CitizensSprite.size(); i++) {
        ...
        CCLOG("shouldMove: %d moved: %d\n", shouldMove,
              SpritetoCitizen[CitizensSprite[i]]->moved);

        if (shouldMove && !SpritetoCitizen[CitizensSprite[i]]->moved) {
            SpritetoCitizen[CitizensSprite[i]]->moved = true;
            int dir = direction(gen);
            float targetX, targetY;
            cocos2d::Vec2 position = CitizensSprite[i]->getPosition();
            targetX = position.x;
            targetY = position.y;
            int deltaX, deltaY;
            if (dir == 0) {
            ...
            if (SpritetoCitizen[CitizensSprite[i]]->animFrames[dir].empty()) {
                CCLOG("Animation frames are empty for direction: %d", dir);
            }

            auto animation = cocos2d::Animation::createWithSpriteFrames(
                SpritetoCitizen[CitizensSprite[i]]->animFrames[dir], 0.36);
            ...
            * 3 / 4) check = 0; if (IsCollisionAtAnyLayer(cocos2d::Vec2(nextX,
            ...
            auto stopAnimate = cocos2d::CallFunc::create([=]() {
                CitizensSprite[i]->stopAction(repeatAnimate);  // stop animation
                CitizensSprite[i]->setTextureRect(
                    cocos2d::Rect(0, dir * 32, 16, 32));
                SpritetoCitizen[CitizensSprite[i]]->moved = false;
                CCLOG("RepeatForever animation stopped after moveAction.");
            });
            auto sequence =
                cocos2d::Sequence::create(moveAction, stopAnimate, nullptr);

            CitizensSprite[i]->runAction(repeatAnimate);
            CitizensSprite[i]->runAction(sequence);
        }
    }
}
